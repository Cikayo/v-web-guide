...
本文档用于公司项目中开发规范，参照百度FEX文档进行编辑，并增加一些自己的限制条件。[百度FEX前端规范连接](https://github.com/fex-team/styleguide/blob/master/css.md)
本文档的目的是规范开发人员CSS风格一致，容易被理解和被维护。
...
必要说明：
  在公司项目中，我们做了一下约定：
1. 类名类似 `v-web-*`，为公共类样式，基本会在所有页面中使用；
2. 类名类似 `web-*`，为手动修改的元素样式，主要针对特定元素进行样式修改；
3. 类名类似 `js-*`，为需要在被 JS 代码使用的元素类名，修改时需要特别谨慎；
4. 类名类似 `ad-*`，为需要在数据统计、广告等插件中使用的类名，禁止自行删除（维护数据统计代码的开发人员除外）； 
5. 类名命名尽量语义化，但是要避免单词过多过长
